#include "device_cat24m01.hpp"

// ============================================================
//  构造 & 初始化
// ============================================================

device_cat24m01::device_cat24m01(inter_i2c_bus* bus, uint8_t addr)
    : _bus(bus)
    , _addr(addr)
{
}

device_cat24m01::~device_cat24m01()
{
}

void device_cat24m01::init()
{
    if (!is_connected())
    {
        _error = ERR_CONNECT;
...
bool device_cat24m01::is_connected()
{
    _bus->lock();
    _bus->start();
...
int device_cat24m01::get_last_error()
{
    int e = _error;
    _error = ERR_OK;
    return e;
...
// 17 位地址 → 7 位器件地址：
//   低 16 位走两个地址字节；bit16（块选择）进器件地址 bit0
//   块 0（0x00000~0x0FFFF）→ 基础地址；块 1（0x10000~0x1FFFF）→ 基础地址 | 0x01
//   ⚠ 与 24LC1025 不同：CAT24M01 的块位在 7 位地址的 bit0
uint8_t device_cat24m01::_device_addr(uint32_t mem_addr) const
{
    if (mem_addr >= BLOCK_SIZE)
    {
        return static_cast<uint8_t>(_addr | 0x01);
    }
    return _addr;
}

bool device_cat24m01::_begin_write(uint32_t mem_addr)
{
    _bus->start();
...
bool device_cat24m01::_wait_write_cycle()
{
...
int device_cat24m01::_write_chunk(uint32_t addr, const uint8_t* buf, uint16_t len)
{
    _bus->lock();
...
uint32_t device_cat24m01::_read_chunk(uint32_t addr, uint8_t* buf, uint32_t len)
{
    _bus->lock();
...
int device_cat24m01::write_byte(uint32_t mem_addr, uint8_t value)
{
    return write_block(mem_addr, &value, 1);
}

int device_cat24m01::write_block(uint32_t mem_addr, const uint8_t* buffer, uint32_t length)
{
    if (!buffer || length == 0)
    {
        _error = ERR_ADDR;
...
int device_cat24m01::set_block(uint32_t mem_addr, uint8_t value, uint32_t length)
{
    if (length == 0)
    {
        _error = ERR_ADDR;
...
int device_cat24m01::update_byte(uint32_t mem_addr, uint8_t value)
{
    // 写前比较：相同则跳过（省一次写周期）
...
uint8_t device_cat24m01::read_byte(uint32_t mem_addr)
{
...
uint32_t device_cat24m01::read_block(uint32_t mem_addr, uint8_t* buffer, uint32_t length)
{
    if (!buffer || length == 0)
    {
        _error = ERR_ADDR;
...
bool device_cat24m01::write_byte_verify(uint32_t mem_addr, uint8_t value)
{
    if (write_byte(mem_addr, value) != ERR_OK) return false;
    return (read_byte(mem_addr) == value);
}

bool device_cat24m01::write_block_verify(uint32_t mem_addr, const uint8_t* buffer, uint32_t length)
{
...
bool device_cat24m01::set_block_verify(uint32_t mem_addr, uint8_t value, uint32_t length)
{
...
void device_cat24m01::set_write_timeout_ms(uint8_t ms)
{
