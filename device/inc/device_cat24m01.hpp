...
// ============================================================
// CAT24M01 1Mbit I2C EEPROM（128KB，分两个 64KB 块）
//
// 来源库 : I2C_CAT24M01 v0.1.3（Rob Tillaart）
// URL    : https://github.com/RobTillaart/I2C_CAT24M01
//
// 移植说明:
//  - 无 float/STL/动态内存/异常；软件 I2C（GPIO 模拟），C++17
//  - 17 位内存地址空间：低 16 位放在两个地址字节（高字节在前），
//    bit16（块选择，数据手册称地址 MSB）放在器件地址 bit0
//    （7 位地址 0x01）——与参考库 _beginTransmission() 一致
//    ⚠ 注意与 24LC1025 的区别：24LC1025 块位在 bit2，CAT24M01 在 bit0
//  - 器件地址 = 0x50 | (A2<<2) | (A1<<1) | (B0<<0)；
//    芯片无 A0 引脚（其位置被块选择位占用），地址 0x50/0x52/0x54/0x56
//  - 页大小 256 字节（参考库 I2C_PAGESIZE_CAT24M01 = 256；
//    移植需求书标注 128，此处以参考库/数据手册为准——若手头
//    数据手册确认为 128，只需改 PAGE_SIZE 常量，逻辑不受影响）
//  - 页写自动分页；写周期用 ACK 轮询等待；
//    读/写均在 0x10000 块边界切分（地址指针块内回绕）
//  - inter_i2c_dev 的 freedom_read/write 只能表达单字节寄存器
//    地址且 ≤8 字节，无法承载 16 位内存地址与 >8 字节块传输，
//    故与 device_eeprom 一致，直接使用 inter_i2c_bus 原语
// ============================================================

class device_cat24m01
{
public:
    // ── I2C 地址（A2/A1 引脚；bit0 为块选择位，勿配）──

    enum Addr : uint8_t
    {
        ADDR_0x50 = 0x50,   // A2=0 A1=0（默认）
        ADDR_0x52 = 0x52,   // A2=0 A1=1
        ADDR_0x54 = 0x54,   // A2=1 A1=0
        ADDR_0x56 = 0x56,   // A2=1 A1=1
    };

    // ── 器件参数 ──

    static constexpr uint32_t DEVICE_SIZE = 131072;   // 128 KB
    static constexpr uint16_t PAGE_SIZE    = 256;     // 页大小（字节）
    ...
    explicit device_cat24m01(inter_i2c_bus* bus, uint8_t addr = ADDR_0x50);

    device_cat24m01(const device_cat24m01&)            = delete;
    device_cat24m01& operator=(const device_cat24m01&) = delete;
